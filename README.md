...
>  浏览器端全栈智能体。MCTS深度规划 · 流水线多Agent调度 · 世界离散验证 · 自进化技能载入 · 心流自主运行。**你的外脑。**
...
-  **心流模式** — 三角色模型（根Agent→监工→干活Agent），审查-修正链，自主对齐→执行→交付全自动闭环
...
├──────────────────────────────────────────────────────────────────┤
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  │
│  │          MCTS    │  │      Pipeline。   │  │        Flow。     │  │        World      │  │        Self-      │  │
│  │           Planner │  │        Engine     │  │       Mode      │  │       Validato r   │  │       Evolution。  │  │
│  │         (UCB1+   │  │       (DAG+      │  │       (3-Role。   │  │       (Nested+    │  │       (Skill        │  │
│  │        Pheromone│  │        Critic)      │  │       +Critique。 │  │        Regions)   │  │     Generate)     │  │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘  │
│            │                      │                       │                       │                      │             │
│  ┌────┴────────────┴─────────────┴───────────┴─────────────┴────┐  │
...
│                  ▼                                               │
│  ┌─────────────────────────────────┐ │
│  │                   心流监工 Agent (Supervisor)                 │ │
│  │   — 中间管理层，全局视野，专属工具 flow_goal/flow_context_new │ │
│  │     /flow_deliver/flow_align_done                             │ │
│  │   — 管理目标清单、审查产出、驱动干活Agent                    │ │
│  │   — 审查-修正链：初稿→审查→修正→…（受 self_critique_rounds    │ │
│  │     控制）                                                    │ │
│  └──────┬──────────────────────────┘ │
│               │ 创建上下文        │ 注入反馈                        │
│               ▼                  ▼                                 │
...
| **强制放权** | 根Agent调用`flow_mode`后立即break停止，不会继续输出或调用其他工具 |
| **上下文保真** | 心流交付后根Agent继承完整内存上下文，非IndexedDB重建 |

**三角色系统指令:**
- `CORE_PROMPT` — 根主Agent（含心流深度工作声明）
- `FLOW_SUPERVISOR_STATIC` — 监工（含目标管理·审查·交付完整SOP）
- `WORKER_PROMPT` — 干活Agent（完全不知道自己身处心流模式）
...
### 1. 单HTML零构建
16364行, 800KB+ 的单个HTML文件。无npm, 无webpack, 无构建工具。所有外部库(18个)通过CDN动态加载, 懒加载策略(`loadOnStart:false`)。
...
### 8. 心流审查-修正链
完全对齐主Agent审查模式: 同一气泡内折叠链(初稿→审查→修正→…), `_createPhaseBlock` + `_addToolBlockAfter` + `thinking-block`完整复用。`settings.self_critique_rounds`全局控制审查轮数, 对齐阶段和执行阶段均受控制。

### 9. 心流上下文保真
交付阶段通过 `_flowWakeupMessages` 旁路, 根Agent醒来时继承内存中完整上下文消息(含`flow_mode` tool_result), 非从IndexedDB重建, 确保对话连续性。
...
> 🛸 Browser-native full-stack AI agent. MCTS planning · Pipeline orchestration · Flow Mode · World simulation · Self-evolution. **Your external brain.**
...
| **Flow Mode** | Three-role model (Root→Supervisor→Worker), critique-revision chain, autonomous align→execute→deliver loop |
